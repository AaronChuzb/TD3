...
// static uint8_t __g_font_buf[294];//如bin文件存在SPI FLASH可使用此buff

static FIL *fd;
static uint8_t *__g_font_buf;
int youyuan_21_init(void)
{

  ft_open(fd, "/fonts/lv_youyuan_21.bin", FA_READ);
  if (!fd)
  {
  ...
  {
    __g_font_buf = heap_caps_malloc(f_size(fd), MALLOC_CAP_SPIRAM);
    if (__g_font_buf == NULL)
    {

      // printf(f_size(fd));
      ESP_LOGE("FONT", "内存申请失败");
    }
    else
    {
      uint32_t read_len;
